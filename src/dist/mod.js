...
Compiler.prototype.traverse = function r(current) {
  if (current.child) {
...
SiblingList.prototype.add = function (component) {
  if (!this.head) {
    this.head = component;
    this.tail = component;
  } else {
...
function Component(label, path, isRoot = false) {
  this.label = label;
  this.path = path;
  this.isRoot = isRoot;
  this.runData();
...
function Parser(root, vue = _CDN) {
  Queue.push(root);
  this.root = root;
  this.vue = vue;
}
Compiler.prototype.write = function w(current) {
...
Initialize.prototype.config = async function (options) {
  try {
    ...
    return new Parser(this.root, vue && vue).parse();
  } catch (error) {
...
async function* walk(
  ...
  for await (const entry of Deno.readDir(root)) {
    ...
    let path1 = join(root, entry.name);
    if (entry.isSymlink) {
      if (followSymlinks) {
...
Initialize.prototype.walk = async function (entry, rootLabel) {
  for await (const file of walk(`${entry}`, {
    exts: ['vue'],
  })) {
    const { path: path1 } = file;
    if (path1.includes(rootLabel)) {
      this.root = new Component(rootLabel, path1, true);
      if (!this.root) throw `there was an error reading ${path1}`;
    } else {
      const regex = new RegExp(/\/(?<label>\w*)(\.vue)$/);
      const label = path1.match(regex)?.groups?.label;
      if (label) Storage1[label] = new Component(label, path1);
      else throw `there was an error reading ${path1}`;
    }
  }
  return true;
};
